...
def compute_similarity_expressions(first_expression, second_expression, current_date):
    if is_date(first_expression)[0]:
        ...
        if is_offset(second_expression)[0]:
            return compute_similarity_offsets(first_expression, is_offset(first_expression)[1], second_expression, is_offset(second_expression)[1], current_date)
        else:
            return 0.0